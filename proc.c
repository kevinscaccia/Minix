 ...
 */
  ...
  if (!rdy_head[q] ) {		/* add to empty queue */
  ...
  else {					/* Add in order of ascending cpu time left */
      if(rdy_head[q]->p_cpu_time_left > rp->p_cpu_time_left){
        ...
        struct proc *index=rdy_head[q];
        while(index->p_nextready->p_cpu_time_left<rp->p_cpu_time_left){
          /*Find place where rp should be*/
          index=index->p_nextready;
        }
        /*insert rp between index and index->p_nextready*/
        rp->p_nextready=index->p_nextready;
        index->p_nextready=rp;
